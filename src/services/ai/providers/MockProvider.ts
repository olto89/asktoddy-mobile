...
export class MockProvider implements AIProvider {
  ...
  private generateMockAnalysis(request: AnalysisRequest): ProjectAnalysis {
    ...
    const isComplexProject = randomProjectType.includes('Extension') || randomProjectType.includes('Conversion');
    ...
    return {
      ...
      permitsRequired: isComplexProject ? ['Building control approval', 'Planning permission may be required'] : ['No permits typically required'],
  ...
  private generateMaterialItems(projectType: string): any[] {
    const materialSets: Record<string, any[]> = {
      'Kitchen Renovation': [
        { name: 'Kitchen Units', quantity: '3 linear meters', unitPrice: 800, totalPrice: 2400, category: 'finishing' },
        { name: 'Worktop', quantity: '4 sq meters', unitPrice: 150, totalPrice: 600, category: 'finishing' },
        { name: 'Appliances', quantity: '1 set', unitPrice: 2500, totalPrice: 2500, category: 'other' },
        { name: 'Tiles', quantity: '8 sq meters', unitPrice: 25, totalPrice: 200, category: 'finishing' }
      ],
      'Bathroom Remodel': [
        { name: 'Bathroom Suite', quantity: '1 set', unitPrice: 800, totalPrice: 800, category: 'finishing' },
        { name: 'Tiles', quantity: '15 sq meters', unitPrice: 30, totalPrice: 450, category: 'finishing' },
        { name: 'Waterproofing', quantity: '1 room', unitPrice: 200, totalPrice: 200, category: 'structural' },
        { name: 'Plumbing Fittings', quantity: 'Various', unitPrice: 300, totalPrice: 300, category: 'plumbing' }
      ],
      'Living Room Extension': [
        { name: 'Bricks', quantity: '2000 units', unitPrice: 0.50, totalPrice: 1000, category: 'structural' },
        { name: 'Roof Materials', quantity: '25 sq meters', unitPrice: 80, totalPrice: 2000, category: 'structural' },
        { name: 'Windows', quantity: '2 units', unitPrice: 600, totalPrice: 1200, category: 'finishing' },
        { name: 'Insulation', quantity: '30 sq meters', unitPrice: 15, totalPrice: 450, category: 'structural' }
      ]
    };

    return materialSets[projectType] || [
      { name: 'Standard Materials', quantity: 'As required', unitPrice: 100, totalPrice: 1000, category: 'other' }
    ];
  }
  ...
  private generateProjectPhases(projectType: string): any[] {
    const phasesSets: Record<string, any[]> = {
      'Kitchen Renovation': [
        { name: 'Design & Planning', duration: '3-5 days', description: 'Kitchen design and material selection' },
        { name: 'Demolition', duration: '1-2 days', description: 'Remove existing kitchen units and appliances' },
        { name: 'First Fix', duration: '3-4 days', description: 'Electrical and plumbing rough work' },
        { name: 'Installation', duration: '5-7 days', description: 'Install units, worktops, and appliances' },
        { name: 'Finishing', duration: '2-3 days', description: 'Tiling, painting, and final touches' }
      ],
      'Bathroom Remodel': [
  ...
  private generateToolRequirements(projectType: string): any[] {
    const toolSets: Record<string, any[]> = {
      'Kitchen Renovation': [
        { name: 'Cordless Drill', category: 'power_tools', dailyRentalPrice: 15, estimatedDays: 5, required: true },
        { name: 'Circular Saw', category: 'power_tools', dailyRentalPrice: 25, estimatedDays: 3, required: true },
        { name: 'Tile Cutter', category: 'power_tools', dailyRentalPrice: 30, estimatedDays: 2, required: false },
        { name: 'Spirit Level', category: 'hand_tools', dailyRentalPrice: 5, estimatedDays: 5, required: true }
      ],
      'Bathroom Remodel': [
        { name: 'Angle Grinder', category: 'power_tools', dailyRentalPrice: 20, estimatedDays: 2, required: true },
        { name: 'Tile Cutter', category: 'power_tools', dailyRentalPrice: 30, estimatedDays: 3, required: true },
        { name: 'Plumbing Tools', category: 'hand_tools', dailyRentalPrice: 15, estimatedDays: 4, required: true }
      ]
    };

    return toolSets[projectType] || [
      { name: 'Basic Tool Kit', category: 'hand_tools', dailyRentalPrice: 20, estimatedDays: 5, required: true },
      { name: 'Power Drill', category: 'power_tools', dailyRentalPrice: 15, estimatedDays: 3, required: true }
    ];
  }
}