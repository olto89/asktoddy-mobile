...
export class UKPricingService {
   ...
   */
  private getToolHirePricing(request: PricingRequest, regionMultiplier: number, seasonalFactor: number): ToolHireRate[] {
    ...
    if (request.tools && request.tools.length > 0) {
      relevantTools = TOOL_HIRE_RATES.filter(tool => 
        request.tools!.some(requestedTool => 
    ...
    return relevantTools.map(tool => ({
      ...
      monthlyRate: tool.monthlyRate ? this.applyPricingFactors(tool.monthlyRate, regionMultiplier, seasonalFactor) : undefined
    }));
  }
   ...
   */
  private getMaterialPricing(request: PricingRequest, regionMultiplier: number, seasonalFactor: number): MaterialPrice[] {
    ...
    if (projectTypeLower.includes('electrical')) {
      relevantMaterials = MATERIAL_PRICES.filter(m => 
        m.category === 'electrical' || m.category === 'structural'
      );
    } else if (projectTypeLower.includes('plumbing') || projectTypeLower.includes('bathroom')) {
      relevantMaterials = MATERIAL_PRICES.filter(m => 
        m.category === 'plumbing' || m.category === 'structural' || m.category === 'finishing'
      );
    } else if (projectTypeLower.includes('kitchen')) {
      relevantMaterials = MATERIAL_PRICES.filter(m => 
        m.category === 'finishing' || m.category === 'electrical' || m.category === 'plumbing'
      );
    }
    ...
    return relevantMaterials.map(material => ({
      ...
      priceRange: {
        ...
        average: this.applyPricingFactors(material.priceRange.average, regionMultiplier, seasonalFactor)
      }
    }));
  }
  ...
  private getAggregatePricing(request: PricingRequest, regionMultiplier: number): AggregateRate[] {
    ...
    return relevantAggregates.map(aggregate => ({
      ...
      pricePerTonne: aggregate.pricePerTonne ? 
        this.applyPricingFactors(aggregate.pricePerTonne, regionMultiplier, 1.0) : undefined,
      pricePerCubicMetre: aggregate.pricePerCubicMetre ? 
        this.applyPricingFactors(aggregate.pricePerCubicMetre, regionMultiplier, 1.0) : undefined
    }));
  }
   ...
   */
  private getLabourPricing(request: PricingRequest, regionMultiplier: number, demandIndex: number): LabourRate[] {
    const projectTypeLower = request.projectType.toLowerCase();
    
    // Filter labour based on project type
    let relevantLabour = [...LABOUR_RATES];
    if (projectTypeLower.includes('electrical')) {
      relevantLabour = LABOUR_RATES.filter(l => 
        l.tradeType.toLowerCase().includes('electrician') || l.tradeType.toLowerCase().includes('general')
      );
    } else if (projectTypeLower.includes('plumbing')) {
      relevantLabour = LABOUR_RATES.filter(l => 
        l.tradeType.toLowerCase().includes('plumber') || l.tradeType.toLowerCase().includes('general')
      );
    } else if (projectTypeLower.includes('roof')) {
      relevantLabour = LABOUR_RATES.filter(l => 
        l.tradeType.toLowerCase().includes('roofer') || l.tradeType.toLowerCase().includes('general')
      );
    }
   ...
   */
  private applyPricingFactors(basePrice: number, regionMultiplier: number, seasonalFactor: number): number {
  ...
  private generateRecommendations(request: PricingRequest, context: any): PricingRecommendation[] {
    ...
    if (context.seasonalFactor > 1.1) {
      recommendations.push({
        type: 'timing',
        message: 'Current season has higher demand. Consider scheduling work for autumn/winter for potential savings',
        priority: 'medium',
        potentialSaving: 300
      });
    }
    ...
    if (bulkMaterials.length > 0) {
      recommendations.push({
        type: 'cost_saving',
        message: 'Consider bulk purchasing for materials like cement and aggregates to reduce unit costs',
        priority: 'medium',
        potentialSaving: 200
      });
    }
